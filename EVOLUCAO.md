...
| 2026-09-18 | **Onboarding: boas-vindas em três telas, e os primeiros passos viram deck.** As boas-vindas moram por cima da home, em `/` (não é gate: o login continua caindo em `/`), só para casal sem jornada nem aporte e uma vez por navegador, pelo cookie `jornada-boas-vindas`. Sem jornada, os passos SÃO o deck da home; com jornada, viram uma linha acima dele. **O convidado sai do beco**: com pedido pendente, só "Esperar a confirmação" — antes a lista mandava criar jornada, e isso fazia `confirm_invite` recusar. Links dos passos corrigidos (`/jornadas/nova`, `/aportes/novo`). Um teste instável achou defeito real: dois toques rápidos em "Próximo" não chegavam à terceira tela. 50/50 e2e, 4 unitários e 3 e2e novos, sabotagens nas duas regras novas | `14e25f2` |
...
| 2026-09-16 | Tinta é a cor da ação; verde e marrom são só as duas pessoas | Uma cor que diz "de quem é o dinheiro" não pode também dizer "aperte". No v2 o verde fazia os dois papéis |
| 2026-09-16 | Aporte se digita num teclado de centavos, e não num campo de texto | Dígitos entrando pela direita já são centavo inteiro: some a classe inteira de erro de leitura ("1.234,56" virando R$ 1,23), e o fluxo sai de Server Action para mutation — uma a menos para reescrever no Expo |
| 2026-09-16 | A chuva de moedas escuta `useAportes`, e o aporte da própria pessoa chega por uma marca em sessionStorage | O realtime já invalidava `useAportes`; nada de assinatura nova. A marca não carrega valor — é só a presença da chave — porque URL e armazenamento não são lugar de número de dinheiro |
| 2026-09-16 | Arte de categoria é Fluent Emoji 3D (MIT), servida de `public/arte/` | Pedido do autor para ficar perto do vídeo. Baixada com autorização, licença junto dos arquivos; nunca imagem de loja (hotlink entregaria IP) |
| 2026-09-18 | O "já vi as boas-vindas" é cookie lido no servidor, e não coluna no banco nem localStorage | Coluna seria segunda verdade sobre o casal; localStorage o servidor não lê, e a home piscaria os slides antes de sumir. Constante do cookie mora em `lib/`: exportada de módulo "use client", ela chegaria ao Server Component como referência, não como texto |
| 2026-09-16 | O vermelho de erro (`#8a3a2a` sobre `#f4e6df`) é a única cor fora das nove, confirmado pelo autor | Cor de estado, não de marca: apagar jornada, sair do plano e apagar conta precisam ser reconhecidos como perigo, e contorno de tinta tiraria esse sinal de quem vai perder dado |
...
| 2026-09-12 | ~~Onboarding é **lista derivada**, não gate obrigatório nem tour guiado~~ — **revista em 2026-09-18**: o autor pediu boas-vindas em três telas. Seguem valendo o "não é gate" (moram em `/`, sem redirect) e a lista derivada (agora em forma de deck) | Gate trava em coisa que a pessoa não termina agora (o convite depende do parceiro aparecer e de ela confirmar, dias depois), contradiz o casal solo que o `handle_new_user` cria, e reprovaria `auth.spec.ts:32` e `auditoria.spec.ts:134`/`:151`, que afirmam `toHaveURL("/")` logo após o login |
| 2026-09-12 | Nenhum estado de onboarding guardado — **exceto, desde 2026-09-18, o cookie "já vi as boas-vindas"**, por navegador e sem valor; os passos continuam derivados dos dados | Cada passo é pergunta que os dados já respondem; uma coluna "tutorial concluído" seria uma segunda verdade para manter alinhada, e mentiria quando alguém apagasse a última jornada |
...
| `DESIGN.md` × tamanhos em uso | A rampa documentada (52/30/26/21/18/14/13/12) é mais estreita que a construída: 15 (linha), 16 (botão), 22, 24 e 28 (título de porta) estão em uso desde o v3, e o detector acusa cada um. Ou a rampa ganha esses degraus, ou as telas se ajustam a ela | Baixa |
| `contributed_at` no teclado | O teclado grava meio-dia UTC da data escolhida, então não existe hora de verdade no aporte. É por isso que a lista de aportes não mostra horário | Baixa |
